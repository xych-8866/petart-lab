...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Printful API error: ${response.statusText}`);
        ...
        for (const product of products) {
            for (const variant of product.variants) {
                ...
                await sql`
                    INSERT INTO printful_products (product_id, variant_id, product_name, product_type, price, image_url, in_stock)
                    VALUES (${product.id}, ${variant.id}, ${product.name}, ${product.type}, ${price}, ${variant.image || ''}, ${variant.in_stock})
